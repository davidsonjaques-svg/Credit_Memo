...
MODEL = "claude-sonnet-4-5"  # matches the model already used in _Fact_Sheet.py
...
def to_fact_sheet_defaults(extraction: ExtractionResult) -> dict:
    """
    Maps extracted financials onto the two-period + mgt-accounts structure
    used in _Fact_Sheet.py's Section 05 (Period 1 = prior year, Period 2 =
    most recent year). Extraction returns periods most-recent-first, so
    index 0 -> Period 2, index 1 -> Period 1.

    Returns a flat dict of floats safe to pass straight into
    ...
    return {
        ...
        "revenue_p2": pick(inc.get("revenue", []), 0),
        "revenue_p1": pick(inc.get("revenue", []), 1),
        "gp_p2": pick(inc.get("gross_profit", []), 0),
        "gp_p1": pick(inc.get("gross_profit", []), 1),
        "ebitda_p2": pick(inc.get("ebitda", []), 0),
        "ebitda_p1": pick(inc.get("ebitda", []), 1),
        "np_p2": pick(inc.get("net_profit", []), 0),
        "np_p1": pick(inc.get("net_profit", []), 1),
        "ca_p2": pick(bs.get("current_assets", []), 0),
        "ca_p1": pick(bs.get("current_assets", []), 1),
        "cl_p2": pick(bs.get("current_liabilities", []), 0),
        "cl_p1": pick(bs.get("current_liabilities", []), 1),
        "debt_p2": pick(bs.get("total_liabilities", []), 0),
        "debt_p1": pick(bs.get("total_liabilities", []), 1),
        "equity_p2": pick(bs.get("total_equity", []), 0),
        "equity_p1": pick(bs.get("total_equity", []), 1),
        "extraction_notes": d.get("extraction_notes", ""),
    }
